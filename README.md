...
The confusion matrix on the left reports the raw numbers while the confusion matrix on the right reports the normalized values. I will only compare to the previous confusion matrices using my normalized values since the dataset I use has a much larger number of samples than KDEF and FER2013. My final model reports very good results. As you can see, most of the dark colors land on the main diagonal squares. As expected, the best performing class ended up being "happy" because it shows up the most out of any other class in my dataset. My confusion matrix shows better results than the confusion matrix on the YOLOv11 model + FER2013 dataset. For example, disgust performs way better in my model using my dataset than on the FER2013 dataset. Even on the KDEF dataset, my confusion matrix has very comparable results on all similar classes which was very unexpected.
<p align="center">
